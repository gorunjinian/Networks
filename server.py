...
class FileServer:
            ...
            while True:
                ...
                try:
                    ...
                    if command == 'UPLOAD':
                        ...
                        self.handle_download(client_socket, request, client_id)
                    elif command == 'LIST':
    ...
    def handle_download(self, client_socket, request, client_id):
        """Handle file download request from client"""
        filename = request.get('filename')
        
        if not filename:
            self.logger.error(f"Filename not provided for download from {client_id}")
            self.send_response(client_socket, {'status': 'error', 'message': 'Filename not provided'})
            return
        
        self.logger.info(f"Download request for {filename} from {client_id}")
        filepath = os.path.join(self.storage_dir, filename)
        
        # Check if file exists
        if not os.path.exists(filepath):
            self.logger.error(f"File {filename} not found, requested by {client_id}")
            self.send_response(client_socket, {'status': 'error', 'message': f'File {filename} not found'})
            return
        ...
        confirmation = json.loads(client_socket.recv(4096).decode('utf-8'))
        if confirmation.get('status') != 'ready':
            self.logger.error(f"Client {client_id} not ready to receive {filename}")
            return
        ...
        self.logger.info(f"File {filename} ({filesize} bytes) sent to {client_id}")
